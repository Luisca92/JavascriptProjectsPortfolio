document.addEventListener('DOMContentLoaded', () => {
    ...
    //move down on loop
    function moveDown() {
        if (!isAtBottomEdge() && !isTaken(currentPosition + width)) {
          undraw()
    ...
    function isTaken(position) {
      return current.some(index => squares[position + index].classList.contains('block2'))
    }
    
    function isAtBottomEdge() {
      return current.some(index => (currentPosition + index + width) >= GRID_SIZE || squares[currentPosition + index + width].classList.contains('block3'))
    }
    ...
    startBtn.addEventListener('click', () => {
      if (timerId) {
        ...
        gameMusic.play(); // Start or resume the music
    ...
    function freeze() {
      ...
      if (current.some(index => squares[currentPosition + index + width].classList.contains('block3') || squares[currentPosition + index + width].classList.contains('block2'))) {
        ...
        freezeSound.currentTime = 0; // Reset the audio to the beginning
        freezeSound.play();
        // make it block2
    ...
    function addScore() {
        for (currentIndex = 0; currentIndex < GRID_SIZE; currentIndex += GRID_WIDTH) {
          ...
          if (row.every(index => squares[index].classList.contains('block2'))) {
            ...
            clearSound.currentTime = 0; // Reset the audio to the beginning
            clearSound.play();
            ...
            // Increase the speed
            clearInterval(timerId);
            timerId = setInterval(moveDown, 1000 - (lines * 50)); // Decrease the interval by 50ms for each completed row
          }
        }
      }
    ...
    startBtn.addEventListener('click', playGameMusic);
